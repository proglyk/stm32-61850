...
void *
	pvPortCalloc(size_t nelem, size_t size);

static void 
	memzero_user( void * buf, size_t size);
...
void *
	pvPortRealloc( void *pv, size_t xWantedSize ) {
		...
		if (pv != NULL)
		{
			...
			if ((pxLink->xBlockSize & xBlockAllocatedBit) != 0)
			{
				...
				if (pvReturn != NULL)
				{
					...
					{
						// Il nuovo posto disponibile e inferiore
						move_size = block_size;
					}
					else
					{
						// Il nuovo posto disponibile e maggiore
...
void *
  ...
  {
    /* The allocated object is too big (note size_t is unsigned). */
    return 0;
  }
  else
  {
    const size_t n = nelem * size;
		char * p = (char *)pvPortMalloc(n);
